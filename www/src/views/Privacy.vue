<template>
  <div class="privacy-policy">

    <h1>PRIVACY POLICY</h1>
    <p class="subtitle">Last updated: February 02, 2026</p>

    <p>
      This Privacy Notice for <strong>JenuelDev</strong> ("we," "us," or "our") describes how and why we might access,
      collect, store, use, and/or share ("process") your personal information when you use our services ("Services"),
      including when you:
    </p>

    <ul>
      <li>
        Visit our website at
        <a href="http://www.Jenuel.Dev" target="_blank">http://www.Jenuel.Dev</a>
        or any website of ours that links to this Privacy Notice.
      </li>
      <li>
        Download and use our mobile application (JenuelDev), our Facebook application (JenuelDev),
        or any other application of ours that links to this Privacy Notice.
      </li>
      <li>
        Engage with us in other related ways, including any marketing or events.
      </li>
    </ul>

    <p>
      <strong>Questions or concerns?</strong>
      Reading this Privacy Notice will help you understand your privacy rights and choices.
      If you do not agree with our policies and practices, please do not use our Services.
      If you still have any questions, contact us at
      <a href="mailto:me@jenuel.dev">me@jenuel.dev</a>.
    </p>

    <hr />

    <h2>SUMMARY OF KEY POINTS</h2>

    <p>
      This summary provides key points from our Privacy Notice. You can find more details in the
      full policy below.
    </p>

    <h3>What personal information do we process?</h3>
    <p>
      We may process personal information depending on how you interact with our Services.
      <a href="#personalinfo">Learn more</a>.
    </p>

    <h3>Do we process sensitive personal information?</h3>
    <p>No. We do not process sensitive personal information.</p>

    <h3>Do we collect information from third parties?</h3>
    <p>No.</p>

    <h3>How do we process your information?</h3>
    <p>
      We process your information to provide, improve, and administer our Services, communicate with you,
      for security and fraud prevention, and to comply with law.
      <a href="#infouse">Learn more</a>.
    </p>

    <h3>How do we keep your information safe?</h3>
    <p>
      We use technical and organizational measures to protect your data, but no system is 100% secure.
      <a href="#infosafe">Learn more</a>.
    </p>

    <h3>What are your rights?</h3>
    <p>
      Depending on your location, you may have rights regarding your personal data.
      <a href="#privacyrights">Learn more</a>.
    </p>

    <hr />

    <h2 id="toc">TABLE OF CONTENTS</h2>
    <ol>
      <li><a href="#infocollect">What Information Do We Collect?</a></li>
      <li><a href="#infouse">How Do We Process Your Information?</a></li>
      <li><a href="#legalbases">What Legal Bases Do We Rely On?</a></li>
      <li><a href="#whoshare">When and With Whom Do We Share Your Information?</a></li>
      <li><a href="#sociallogins">How Do We Handle Social Logins?</a></li>
      <li><a href="#inforetain">How Long Do We Keep Your Information?</a></li>
      <li><a href="#infosafe">How Do We Keep Your Information Safe?</a></li>
      <li><a href="#infominors">Do We Collect Information From Minors?</a></li>
      <li><a href="#privacyrights">What Are Your Privacy Rights?</a></li>
      <li><a href="#DNT">Do-Not-Track Controls</a></li>
      <li><a href="#uslaws">US Privacy Rights</a></li>
      <li><a href="#policyupdates">Updates to This Policy</a></li>
      <li><a href="#contact">How to Contact Us</a></li>
      <li><a href="#request">How to Review, Update, or Delete Your Data</a></li>
    </ol>

    <hr />

    <h2 id="infocollect">1. WHAT INFORMATION DO WE COLLECT?</h2>

    <h3 id="personalinfo">Personal Information You Disclose to Us</h3>
    <p>
      We collect personal information you voluntarily provide when you register, contact us,
      or use our Services.
    </p>

    <p><strong>Examples of information we may collect:</strong></p>
    <ul>
      <li>Names</li>
      <li>Email addresses</li>
      <li>Billing addresses</li>
      <li>Payment information (processed via Stripe, Paymongo, or PayPal)</li>
      <li>Authentication data</li>
    </ul>

    <h3>Payment Data</h3>
    <p>
      Payment data is handled and stored by:
      <a href="https://stripe.com/privacy" target="_blank">Stripe</a>,
      <a href="https://www.paymongo.com/privacy" target="_blank">PayMongo</a>, and
      <a href="https://www.paypal.com/us/legalhub/paypal/privacy-full" target="_blank">PayPal</a>.
    </p>

    <h3>Social Media Login Data</h3>
    <p>
      If you log in via Facebook or other platforms, we may receive basic profile information such as
      your name, email, and profile picture.
    </p>

    <h3>Application Data</h3>
    <ul>
      <li>
        <em>Push Notifications:</em> You may opt out in your device settings.
      </li>
    </ul>

    <hr />

    <h2 id="infouse">2. HOW DO WE PROCESS YOUR INFORMATION?</h2>

    <p>We process your data to:</p>
    <ul>
      <li>Manage your account</li>
      <li>Provide and improve our Services</li>
      <li>Ensure security and prevent fraud</li>
      <li>Comply with legal obligations</li>
    </ul>

    <hr />

    <h2 id="legalbases">3. LEGAL BASES FOR PROCESSING</h2>

    <p>We rely on:</p>
    <ul>
      <li>Your consent</li>
      <li>Legal obligations</li>
      <li>Protection of vital interests</li>
      <li>Legitimate business interests</li>
    </ul>

    <hr />

    <h2 id="whoshare">4. WHEN AND WITH WHOM DO WE SHARE YOUR DATA?</h2>

    <p>We may share your data in cases such as:</p>
    <ul>
      <li>Business transfers (mergers, acquisitions)</li>
      <li>Legal compliance</li>
      <li>With trusted service providers</li>
    </ul>

    <hr />

    <h2 id="sociallogins">5. SOCIAL LOGINS</h2>

    <p>
      We only use data received from social platforms for account authentication
      and service delivery.
    </p>

    <hr />

    <h2 id="inforetain">6. DATA RETENTION</h2>

    <p>
      We retain your personal information only as long as necessary or as required by law.
    </p>

    <hr />

    <h2 id="infosafe">7. DATA SECURITY</h2>

    <p>
      We use reasonable security measures, but no system is completely secure.
    </p>

    <hr />

    <h2 id="infominors">8. MINORS</h2>

    <p>
      We do not knowingly collect data from individuals under 18.
    </p>

    <hr />

    <h2 id="privacyrights">9. YOUR PRIVACY RIGHTS</h2>

    <p>
      Depending on your location, you may have rights to access, correct, delete,
      or restrict your data.
    </p>

    <hr />

    <h2 id="DNT">10. DO-NOT-TRACK</h2>

    <p>
      We currently do not respond to Do-Not-Track signals.
    </p>

    <hr />

    <h2 id="uslaws">11. US PRIVACY RIGHTS</h2>

    <p>
      Certain US states grant additional privacy rights. Please contact us for details.
    </p>

    <hr />

    <h2 id="policyupdates">12. POLICY UPDATES</h2>

    <p>
      We may update this policy from time to time. Please review periodically.
    </p>

    <hr />

    <h2 id="contact">13. CONTACT US</h2>

    <p>Email: <a href="mailto:me@jenuel.dev">me@jenuel.dev</a></p>

    <p>
      JenuelDev<br />
      Mankayan, Benguet, 2608<br />
      Philippines
    </p>

    <hr />

    <h2 id="request">14. REVIEW, UPDATE, OR DELETE YOUR DATA</h2>

    <p>
      To request access, correction, or deletion of your data, contact us at:
      <a href="mailto:me@jenuel.dev">me@jenuel.dev</a>
    </p>

    <p>
      This Privacy Policy was created using
      <a href="https://termly.io/products/privacy-policy-generator/" target="_blank">
        Termly’s Privacy Policy Generator
      </a>.
    </p>

  </div>
</template>
